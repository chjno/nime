...
//      if ((j != 11) && (j != 18) && (j != 19)){
        if (digitalRead(inputPins[j]) == HIGH){
          if (sampleState[i] != j){
//          if (sampleState[i] == 0){ // if not using normal plugs
            Serial.print(String(i + 1));
            Serial.print(' ');
            Serial.println(String(j + 2));
            sampleState[i] = j;
//            sampleState[i] = 1; // if not using normal plugs
          }
          
        } else {
          if (sampleState[i] != i + 31){
//          if (sampleState[i] == 1){ // if not using normal plugs
            Serial.print(String(i + 1));
            Serial.print(' ');
            Serial.println(String(i + 31)); // if not using normal plugs
            sampleState[i] = i + 31;
//            Serial.println('0');            // if using normal plugs
//            sampleState[i] = 0;
          }
